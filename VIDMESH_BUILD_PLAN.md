## Agent prompt: build the Vidmesh monorepo from zero

You are an expert protocol engineer and full-stack developer. Your task is to build
**Vidmesh** — a decentralized video protocol and its reference implementations — as a
...
A companion document, `VIDMESH_SPEC_PROPOSAL.md`, is provided alongside this plan and
...
Vidmesh is "many gateways, one substrate" video:
...
│   └── 011-threat-model.md
├── crates/
│   ├── vidmesh-kernel/        ← the permanent library
│   ├── vidmesh-relay/         ← axum websocket+http relay binary
│   ├── vidmesh-wasm/          ← wasm-bindgen wrapper over kernel
│   └── vidmesh-node/          ← Tauri 2 scaffold (Phase 8)
├── packages/
...
│   └── site/                  ← vidmesh.org landing page, static, see §14
└── tools/
...
| 2 | `vidmesh-kernel` | all §6 APIs implemented; unit tests + test vectors pass; no `unsafe` |
| 3 | `vidmesh-wasm` + `kernel-ts` | browser + node can verify/create records identically to Rust tests |
| 4 | `vidmesh-relay` | relay passes conformance suite; two relays gossip records |
...
Transcribe and expand `VIDMESH_SPEC_PROPOSAL.md` into the `spec/` folder. Rules:
...
## 6. Phase 2 — `vidmesh-kernel` public API
...
`vidmesh-wasm` exposes the kernel via wasm-bindgen. `packages/kernel-ts` wraps it in
...
wordmark "vidmesh" in lowercase, set in an open-license geometric sans (embed as
