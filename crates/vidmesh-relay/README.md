...
API (`vidmesh_kernel::Record`, `ChunkTree`) that is specified but not
...
```sh
cargo run -p vidmesh-relay -- path/to/relay.json
# or
VIDMESH_RELAY_CONFIG=path/to/relay.json cargo run -p vidmesh-relay
```
...
depends on `vidmesh_kernel::Record`/`ChunkTree`, which are specified
