//! wasm-bindgen bindings over `evermesh-kernel`.
//!
...
use evermesh_kernel::{codec, kinds, Identity, IdentityId, Keypair, Record, RecordId};
use wasm_bindgen::prelude::*;

fn js_err(e: evermesh_kernel::Error) -> JsError {
...
pub fn build_record(
    ...
    for entry in refs_arr {
        ...
        refs.push(evermesh_kernel::Ref {
            ref_type,
            hash: take32(hash, "ref hash")?,
        });
    }
    let body = codec::from_json(body_json).map_err(js_err)?;
    let record = evermesh_kernel::RecordBuilder::new(kind as u64)
...
pub fn hash_blob(bytes: &[u8]) -> Vec<u8> {
    evermesh_kernel::blob::hash_blob(bytes).as_bytes().to_vec()
}
...
pub fn sign_derivation(
    ...
    let original = evermesh_kernel::BlobId(take32(original, "original blob id")?);
    let rendition_id = evermesh_kernel::BlobId(take32(rendition, "rendition blob id")?);
...
pub fn verify_chunk(
    ...
    let siblings: Vec<[u8; 32]> = proof
        ...
        .collect();
    evermesh_kernel::blob::verify_chunk(&root, n_chunks as usize, index as usize, chunk, &siblings)
        .map_err(js_err)
}
...
impl BlobHasher {
    ...
    pub fn update(&mut self, bytes: &[u8]) -> Result<(), JsError> {
        ...
        let mut rest = bytes;
        let chunk_size = evermesh_kernel::blob::CHUNK_SIZE;
        while !rest.is_empty() {
            ...
            if self.pending.len() == chunk_size {
                self.leaves
                    .push(evermesh_kernel::blob::leaf_hash(&self.pending));
                self.pending.clear();
            }
        }
        Ok(())
        ...
        self.root = if level.is_empty() {
            ...
            while level.len() > 1 {
                ...
                for pair in level.chunks(2) {
                    match pair {
                        [l, r] => next.push(evermesh_kernel::blob::node_hash(l, r)),
                        [odd] => next.push(*odd),
                        _ => {}
                    }
                }
...
mod tests {
    ...
    fn blob_hasher_matches_kernel() {
        let data = vec![7u8; evermesh_kernel::blob::CHUNK_SIZE + 123];
        ...
        assert_eq!(
            h.id_hex().unwrap(),
            evermesh_kernel::blob::hash_blob(&data).to_hex()
        );
        let tree = evermesh_kernel::ChunkTree::from_bytes(&data);
