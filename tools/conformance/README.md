...
The Vidmesh conformance suite: deterministic JSON fixture vectors covering
...
the `vidmesh-kernel` crate in-process, `@vidmesh/kernel` under Node (via a
small child-process harness), and a live `vidmesh-relay` over its
...
| `record-invalid` | `cbor_hex`, `expected_error`, `layer` | The bytes MUST be rejected. `expected_error` is one of `cbor`, `non-canonical`, `envelope`, `signature`, `unknown-algorithm`, `kind` (mirrors `vidmesh_kernel::Error`). `layer` is `"envelope"` (checkable by `Record::from_cbor` + `Record::verify` alone — every target checks these today) or `"kind"` (only checkable by `vidmesh_kernel::kinds::validate`, not yet wired into this runner — see below). |
...
`vidmesh_kernel::codec::Value` per spec 003/004's schemas, **not** via
`vidmesh_kernel::kinds` (that module is being completed in parallel and is
...
# Against @vidmesh/kernel under Node (requires crates/vidmesh-wasm built
# into packages/kernel-ts/wasm/, and Node >= 22.6):
cargo run --bin vidmesh-conformance -- run --target node
...
`vidmesh_kernel::kinds` lands), or a `bundle` vector against the node
target (`@vidmesh/kernel` exposes no bundle API), or any non-record
...
1. **`vidmesh_kernel::kinds` is not part of the compiled kernel crate
   yet.** `crates/vidmesh-kernel/src/kinds/mod.rs` references
   `claims`, `compliance`, `infra`, `live`, `social`, `trust` submodules
   that do not exist on disk yet (only `content.rs` does), and
   `crates/vidmesh-kernel/src/lib.rs` does not declare `pub mod kinds;`
   ...
   *skips* rather than checks (see `src/kernel_target.rs`'s `// kinds`
   ...
   `crates/vidmesh-wasm/src/lib.rs` already assumes `kinds` exists
   (`use vidmesh_kernel::{..., kinds, ...}`) and calls
   `kinds::validate` in `validate_kind` — so `vidmesh-wasm` likely does
   not compile today either, independent of this suite.

2. **The WASM `identity.verifyChain` binding cannot express
   contest-window finality.** `crates/vidmesh-wasm/src/lib.rs`'s
   ...
   `vidmesh_wasm::verify_chain` → `kernel-ts`'s `identity.verifyChain`
...
4. **The `bundle/roundtrip` and `bundle/corrupted-blob-item` fixtures'
   ...
   by `Bundle::export`.** `vidmesh_kernel::bundle::PART_SPLIT_THRESHOLD`
