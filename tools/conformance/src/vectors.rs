...
//!   vocabulary as [`vidmesh_kernel::Error`]: `cbor`, `non-canonical`,
...
//!   (`vidmesh_kernel::kinds::validate`) runs, which is not wired into
...
/// error reporting. Mirrors [`vidmesh_kernel::Error`]'s variants.
pub mod error_class {
    ...
    /// Classify a [`vidmesh_kernel::Error`] into this vocabulary.
    pub fn classify(e: &vidmesh_kernel::Error) -> &'static str {
        use vidmesh_kernel::Error;
        match e {
            Error::Cbor(_) => CBOR,
...
pub enum Layer {
    ...
    Envelope,
    /// Only kind-level validation (`vidmesh_kernel::kinds::validate`)
