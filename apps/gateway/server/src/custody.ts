...
const HKDF_INFO = Buffer.from("vidmesh:custody:v1", "utf-8");
...
export class CustodyService {
   ...
   * per the build plan, even though `@vidmesh/kernel` already exposes
   * `signDerivation` (verified against crates/vidmesh-wasm/src/lib.rs:
   * it builds the exact `[original, rendition, codec, width, height,
   * bitrate]` statement and signs `"vidmesh:derivation:v1" ||
